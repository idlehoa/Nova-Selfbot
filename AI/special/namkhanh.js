...
function removeDiacritics(str) {
  ...
  export function namKhanhSpecial(userMessage, aiOriginalReply) {
    // (1) Xử lý đơn giản trước: bỏ dấu, chuẩn hóa về thường, bỏ ký tự đặc biệt
    let normalized = removeDiacritics(userMessage)
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, " ")
      .replace(/\s+/g, " ")
      .trim();
    ...
    if (
      ...
      const reply = shuffled.slice(0, Math.floor(Math.random() * 2) + 2).join(' ');
      return reply;
    }
